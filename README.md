...
**架构（阶段 1～2）**：编辑仍基于 Qt 富文本（`QTextDocument`）；**分页预览、PDF/打印，以及活页页数与页缝**走自研轻量布局引擎（`engine/`：`DocumentModel` → `LayoutEngine` → `LayoutPainter`）。**内置 DOCX** 已闭环：`DOCX ⇄ DocumentModel ⇄ QTextDocument`（`DocxImporter` / `DocxExporter` + `QTextAdapter`）。增强路径仍可用 mammoth/html2docx。编辑器内断行仍由 Qt 排版，与 PDF 行边界可能有细微差别——不是「以引擎为中心的所见即所得编辑器」，但**页数已与引擎对齐**（见 [`docs/pagination_eval.md`](docs/pagination_eval.md)）。
...
单测覆盖：空页、长文分页、页数与 Painter API 一致、颜色/图片/表格适配、**Model 闭环导入**。
...
依赖：Qt 6.11+（Widgets + PrintSupport）、macOS、zlib。  
...
会覆盖 DOCX 往返、LayoutEngine 分页，以及 **活页页数 vs 引擎页数**（含图 / 多页表样例）。  
评测表：[`docs/pagination_eval.md`](docs/pagination_eval.md)（Δ活页=0；几何估算短样例 ±1 / 长文·图·多页表 ±3）。  
论文提纲：[`docs/thesis_ch4_ch5_outline.md`](docs/thesis_ch4_ch5_outline.md)。
